...
class GradioOutputs():
    ...
    def model_result(self, experiment_name, image) -> str:
        ...
        """
        result = requests.get(f"http://backend.docker:8000/v1/getinference/{experiment_name}/{image}")
        return result.text
    ...
    def training_result(self,experiment_name) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        """
        data = pd.read_sql_query("select learning_rate, momentum, loss from experiment_data where experiment_name = %(value)s", self.connection, params={"value": experiment_name})
        data = pd.concat([data.drop(['loss'], axis=1), pd.DataFrame(ast.literal_eval(data['loss'][0]))], axis=1)
...
def main():
    ...
    gr.TabbedInterface([gr.Interface(
    fn=go.model_result,
    inputs=[gr.Dropdown(choices=models, value=models[0]),
        gr.Dropdown(choices=images, value=images[0])],
    outputs="text"), gr.Interface(fn=go.training_result, inputs=[gr.Dropdown(choices=models, value=models[0])],outputs=[gr.Timeseries(x='Step',y='Value'), gr.Dataframe()])],tab_names=['model_inference','training_metrics']).launch(server_port=8501, server_name="0.0.0.0")
